...
`netplot` recieves the MSEA result, the metabolite set and `shared.metabolite` argument and plots the pvalue in the MSEA result as the node color on the network of the metabolite group. The size of node indicates the number of metabolite queries contained in the metabolite group. The `shared.metabolite` argument decides how to connect the metabolite group with the edge. In the example below, if there are more than 20 metabolites common among groups, we connect these groups with edge. 

By default, `netplot` performs visualization using the visNetwork package, but you can add `sendto` argument and make the output destination Cytoscape.
